...
test.describe('reduce()', () => {
  test('Check the defaults', async ({ page }) => {
    ...
    await test.step('Exists and is a function', async () => {
      ...
      const type = await page.evaluate(() => typeof window.table.reduce);
      expect(type).toBe('function');
    });
    await test.step('Returns whatever function returns', async () => {
      const isTrue = await page.evaluate(() => {
        let params;
        const result = window.table.column(0).data().reduce(function() {
          params = arguments;
          window._reduceParams = params;
          return true;
        });
        return result;
      });
    ...
    await test.step('Correct params to function', async () => {
      const [len, t0, t1, t2, t3] = await page.evaluate(() => {
        const params = window._reduceParams;
        return [
          params.length,
          typeof params[0],
          typeof params[1],
  ...
  test('Functional tests', async ({ page }) => {
    ...
    await test.step('Reducing 1d data', async () => {
      ...
      const result = await page.evaluate(() => {
        let count = 0;
        const reduced = window.table.column(0).data().reduce(function(acc, value, idx) {
          count++;
