...
import { _each, _isPlainObject, _map, _pluck } from "../core/core.internal";
import { _dt_api } from "./api.base";
import { _dt_settings } from "./api.static";
import { _dt_ext } from "../ext/ext";
import $ from "jquery";

export function _registerApis_core(register, registerPlural) {
  ...
  register("destroy()", function (remove) {
    ...
    return this.iterator("table", function (settings) {
      ...
      $(jqTable)
        .find("th, td")
        .removeClass(
          _map(_dt_ext.type.className, function (v) {
            return v;
          }).join(" ")
        );
