...
import $ from "jquery";
import { _each, _extend, _isPlainObject } from "./core.internal";
...
export function _fnRenderer(settings, type) {
  var renderer = settings.renderer;
  var host = _dt_ext.renderer[type];
...
export function _fnEscapeObject(obj) {
  if (_dt_ext.escape.attributes) {
    _each(obj, function (key, val) {
      obj[key] = _dt_util._escapeHtml(val);
    });
  }
...
export function _fnLengthOverflow(settings) {
 ...
 * @returns String
 */
export function _fnMacros(settings, str, entries) {
