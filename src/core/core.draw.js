import { _fnGetCellData, _fnWriteCell, _fnGetDataMaster } from "./core.data";
import { _addClass, _unique, _pluck, _range, _isPlainObject, _each } from "./core.internal";
import { _fnCallbackFire, _fnLog, _fnDataSource, _fnRenderer, _fnEscapeObject } from "./core.support";
import { _fnFeatureHtmlTable } from "./core.scrolling";
import { _fnColumnTypes, _fnColumnOptions, _fnVisbleColumns } from "./core.columns";
import { _dt_util } from "../api/api.util";
import { _fnProcessingDisplay, _processingHtml } from "./core.processing";
import { _fnSort } from "./core.sort";
 ...
 *  @memberof DataTable#oApi
 */
export function _fnRowAttributes(settings, row) {
  var tr = row.nTr;
  var data = row._aData;
 ...
 *  @memberof DataTable#oApi
 */
export function _fnBuildHead(settings, side) {
 ...
 */
export function _fnHeaderLayout(settings, source, incColumns) {
  var row, column, cell;
  var local = [];
  var structure = [];
  var columns = settings.aoColumns;
 ...
 *  @memberof DataTable#oApi
 */
export function _fnDrawHead(settings, source) {
 ...
 *  @memberof DataTable#oApi
 */
export function _fnDraw(oSettings, ajaxComplete) {
  ...
  if (aiDisplay.length !== 0) {
    ...
    for (var j = iStart; j < iEnd; j++) {
      ...
      for (var i = 0; i < columns.length; i++) {
        ...
        _addClass(td, _dt_ext.type.className[col.sType]); // auto class
 ...
 *  @memberof DataTable#oApi
 */
export function _fnReDraw(settings, holdPosition, recompute) {
...
function _layoutResolve(settings, row) {
  var getFeature = function (feature, opts) {
    if (!_dt_ext.features[feature]) {
      _fnLog(settings, 0, "Unknown feature: " + feature);
    }

    return _dt_ext.features[feature].apply(this, [settings, opts]);
  };
...
function _fnLayoutDom(settings, dom, insert) {
  ...
  for (var i = 0; i < parts.length; i++) {
    ...
    if (option == "<") {
      ...
      _dt_ext.feature.forEach(function (feature) {
          ...
          if (unique) {
            ...
            if (isHeader) {
              ...
              if (!columnDef.sTitle && unique) {
                columnDef.sTitle = _dt_util._stripHtml(cell.innerHTML);
