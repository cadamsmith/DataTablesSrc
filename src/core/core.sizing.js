import { _fnGetColumns, _fnColumnsSumWidth, _fnAdjustColumnSizing } from "./core.columns";
import { _dt_util } from "../api/api.util";
import { _fnCallbackFire } from "./core.support";
import { _fnGetRowDisplay } from "./core.draw";
...
export function _fnCalculateColumnWidths(settings) {
  ...
  for (i = 0; i < visibleColumns.length; i++) {
    ...
    var autoClass = _dt_ext.type.className[column.sType];
...
function _fnGetMaxLenString(settings, colIdx) {
  ...
  if (!column.maxLenString) {
    ...
    for (var i = 0, ien = settings.aiDisplayMaster.length; i < ien; i++) {
      ...
      s = _dt_util._stripHtml(cellString).replace(/&nbsp;/g, " ");
 ...
 *  @memberof DataTable#oApi
 */
export function _fnStringToCss(s) {
  if (s === null) {
    return "0px";
  }
